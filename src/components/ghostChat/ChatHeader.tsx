...
export const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  const requestNotifyPermission = async () => {
    ...
    if (permission === 'granted') {
      ...
      new Notification('Ghost Chat', { body: 'Secure Channel Active' });
    } else {
      toast.error('DENIED');
    }
  };
    ...
    <div className="bg-xmr-surface border-b border-xmr-border p-3 flex justify-between items-center shadow-sm z-10 shrink-0">
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <div className="overflow-hidden">
          <div className="flex items-center gap-2">
            {isEditingName ? (
              <div className="flex items-center">
                <input
                  autoFocus
                  className="bg-black/20 border-b border-xmr-green text-xmr-green text-sm font-mono w-24 focus:outline-none select-text"
                  value={tempName}
          ...
          <p className="text-[10px] text-xmr-dim font-mono truncate w-24 md:w-auto select-text">{partner.fingerprint}</p>
        </div>
      </div>
