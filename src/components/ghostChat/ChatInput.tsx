...
export const ChatInput: React.FC<ChatInputProps> = ({ onSend, onManualDecrypt, onPasteInput }) => {
  ...
  return (
    <div className={`p-2 md:p-3 border-t z-10 shrink-0 transition-colors ${isCiphertext ? 'bg-yellow-500/10 border-yellow-500/50' : 'bg-xmr-surface border-xmr-border'
      }`}>
      <div className="relative flex items-end gap-2">
        ...
        >
          ...
          <span className="text-[8px] font-mono">{t('ghostChat.paste')}</span>
        </button>
        ...
        <textarea
          ...
          className={`w-full bg-xmr-base border rounded-sm p-3 text-sm font-mono focus:outline-none resize-none h-12 md:h-16 scrollbar-hide transition-colors select-text ${isCiphertext
