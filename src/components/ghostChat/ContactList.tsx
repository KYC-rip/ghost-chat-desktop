...
import { Plus, Trash2, Key, Shield, LogOut, Copy, QrCode, Edit2, Check, Search, X } from 'lucide-react';
...
export const ContactList: React.FC<ContactListProps> = ({
  ...
  const [qrMode, setQrMode] = useState<'show' | null>(null);
  ...
  return (
    <>
      <div className="w-full md:w-80 flex flex-col border-r border-xmr-border bg-xmr-surface/80 backdrop-blur-sm h-full relative z-20">
        ...
        <div className="p-3 border-b border-xmr-border shrink-0">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-xmr-dim/10 border border-xmr-border text-xs px-2 py-1 pl-7 text-xmr-green font-mono focus:border-xmr-green focus:outline-none rounded-sm select-text"
            />
            {searchQuery && (
              <button
                onClick={() => onSearch('')}
        ...
        {/* 2. Add Area */}
        {isAdding && (
          <div className="p-3 bg-xmr-base/50 border-b border-xmr-border">
            <textarea
              autoFocus
              value={newKeyInput}
              onChange={(e) => setNewKeyInput(e.target.value)}
              placeholder={t('ghostChat.pasteInvitePacket')}
              className="w-full h-16 bg-xmr-green/10 border border-xmr-border text-xs p-2 text-xmr-green font-mono resize-none focus:border-xmr-green focus:outline-none mb-2 rounded-sm select-text"
            />
            ...
            <div className="flex-1 overflow-hidden min-w-0">
              {isEditingName ? (
                <div className="flex items-center gap-1">
                  <input
                    autoFocus
                    className="w-full bg-xmr-green/10 border-b border-xmr-green text-sm text-xmr-green font-mono focus:outline-none py-0.5 select-text"
                    value={tempName}
                ...
                <div className="group cursor-pointer" onClick={startEditing}>
                  ...
                  <p
                    className="text-[9px] text-xmr-dim font-mono truncate hover:text-xmr-green transition-colors mt-0.5 flex items-center gap-1 select-text"
